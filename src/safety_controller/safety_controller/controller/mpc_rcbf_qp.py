...
class MPC_RCBF_QP:
    ...
    def update_n_solve(self, ref_states, cur_states, obs_states, pre_input):
        ...
        sol_dict['optimal']    = qp_sol_dict['optimal']        # whether the solution is optimal or not
...
class CBFQP:
    ...
    def add_cbf_constraint(self):
        ...
        for obs_idx, obs in enumerate(self.obstacles):  # Loop through all obstacles
            ...
            # Compute gradients
            dh_dx = (x - obs_x) / casadi.sqrt((x - obs_x)**2 + (y - obs_y)**2)
            dh_dy = (y - obs_y) / casadi.sqrt((x - obs_x)**2 + (y - obs_y)**2)
            dB_dx = dB_dh * dh_dx
            dB_dy = dB_dh * dh_dy

            # grad_h = casadi.vertcat(grad_h_x, grad_h_y, 0, 0)
            L_f_B = dB_dx @ f  # Lie derivative of h along f
            L_g_B = dB_dy @ g  # Lie derivative of h along g

            # Define constraint
            cbf_constraint = L_f_B + L_g_B @ casadi.vertcat(u1, u2)  - self.GAMMA*h
