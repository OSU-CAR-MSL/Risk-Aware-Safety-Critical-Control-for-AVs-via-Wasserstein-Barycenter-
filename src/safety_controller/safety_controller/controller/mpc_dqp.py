...
class MPC_CBF_DQP:
    ...
    def update_n_solve(self, ref_states, cur_states, obs_states, pre_input):
        ...
        acc   = mpc_sol_dict['u_control']
        steer = mpc_sol_dict['u_control']
        # prediction point
        z_pred = mpc_sol_dict['z_mpc']
        z_predy = mpc_sol_dict['z_mpc']
        u_nom = mpc_sol_dict['u_mpc']

        self.qp.update_nominal_obs(u_nom, obs_states, z_pred)
...
class CBFQP:
    ...
    def __init__(self,para, obs_init):
        ...
        self._parameter_assign(para)
        self.u_var = self.qp_opti.variable(self.N,2)
        self.u_nom = self.qp_opti.parameter(self.N, 2)

        self.optiobs = self.qp_opti.parameter(self.numofobs * 4)
        # self.optiobs_x = self.optiobs[::4]
        # self.optiobs_y = self.optiobs[1::4]
        self.x_var = self.qp_opti.parameter(self.N,4)
    ...
    def define_cost(self):
        ...
        for i in range(self.N):
            self.cost += 0.5 * casadi.mtimes((self.u_var[i,0] - self.u_nom[i,0]).T, casadi.mtimes(1, (self.u_var[i,0] - self.u_nom[i,0])))
            self.cost += 0.5 * casadi.mtimes((self.u_var[i,1] - self.u_nom[i,1]).T, casadi.mtimes(1, (self.u_var[i,1] - self.u_nom[i,1])))
            # self.cost = 0.5 * casadi.mtimes((self.u_var[i,:] - self.u_nom[i,:]).T, casadi.mtimes(self.H, (self.u_var[i,:] - self.u_nom[i,:])))
    ...
    def define_dynamics(self, x, y, psi, v, a, delta):
        ...
        g = casadi.vertcat(
            ...
            casadi.horzcat(1, 0),                # Control influence on psi
            casadi.horzcat(0, 1)                 # Control influence on v
        )
        return f, g
    ...
    def add_cbf_constraint(self):
        ...
        for i in range(self.N):
            beta = casadi.atan( self.L_R / (self.L_F + self.L_R) * casadi.tan(u2[i]) )
            print('i',i)
            xn = x[i] +  u1[i] * casadi.cos(psi[i] + beta)
            yn = y[i] +  u1[i] * casadi.sin(psi[i] + beta)
            print('xn', xn)
        ...
        for obs_idx, obs in enumerate(self.obstacles):  # Loop through all obstacles
            ...
            for i in range(self.N-1):
                ...
                cbf_constraint = hn - (1 - self.GAMMA) * h
                self.qp_opti.subject_to(cbf_constraint > 0)  # Apply constraint

            # Define h(x)
            # h = casadi.sqrt((x - obs_x)**2 + (y - obs_y)**2) - (self.VEHICLE_R + OBSTACLE_R + self.SAFETY_DIS)
            # hn = casadi.sqrt((xn - obs_x)**2 + (yn - obs_y)**2) - (self.VEHICLE_R + OBSTACLE_R + self.SAFETY_DIS)
            # cbf_constraint = hn - (1 - self.GAMMA) * h
            # self.qp_opti.subject_to(cbf_constraint > 0)  # Apply constraint
            ...
            # # Define constraint
            # cbf_constraint = L_f_h + L_g_h @ casadi.vertcat(u1, u2)  + (1-self.GAMMA)*h
