...
class Trajectory:
    ...
    def __init__(self, para):
        ...
        for speed in self.v_ref:
            self.V_REF = speed
        self.path = os.path.join(parent_dir, "trajectory_data", self.path)


    
    def waypoint_gen(self):
        # read trajectory and finalized local coordinate waypoints
        self.waypoints_lla = pd.read_csv(self.path, header=None) #read the reference lat and lon here
        ...
        AX, AY  = functions.lla_to_enu(wps_lat, wps_lon, wps_alt, wps_lat[0], wps_lon[0], wps_alt[0])
