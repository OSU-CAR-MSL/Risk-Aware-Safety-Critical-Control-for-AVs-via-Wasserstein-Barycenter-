...
class Analysis():
    ...
    def extract_info(self, filename):
        ...
        match = re.search(r"\['(\d+)'\]_\[(.*?)\]_\[(.*?)\]", filename)

        if match:
            scenario, controller, velocity = match.groups()
            try:
                velocity = float(velocity)  # Convert velocity to float
            except ValueError:
                velocity = None  # Handle potential errors gracefully
            return scenario, controller, velocity
    ...
    def grouped(self, files):
        ...
        for file_path in files:
            ...
            scenario, controller, velocity = self.extract_info(filename)

            if scenario and velocity and controller is not None:
                grouped_files[scenario][velocity][controller].append(file_path)
    ...
    def plot_controller_info(self, grouped_files):
        ...
        filtered_config_files = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
        filtered_mpc_files = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
        filtered_rt_files = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))

        for scenario, velocities in grouped_files.items():
            for velocity, controllers in velocities.items():
                for controller, files in controllers.items():
                    ...
                    if mpc_files and rt_files:
                        filtered_config_files[scenario][velocity][controller] = config_files
                        filtered_mpc_files[scenario][velocity][controller] = mpc_files
                        filtered_rt_files[scenario][velocity][controller] = rt_files
        unique_controllers = sorted(
            {controller for velocities in filtered_mpc_files.values() 
                        for controllers in velocities.values() 
    ...
    def plot_mpc_traj_gif(self, filtered_mpc_files, filtered_rt_files, filtered_config_files):
        for scenario, velocities in filtered_mpc_files.items():
            for velocity, controllers in sorted(velocities.items()):
                for i, (controller, mpc_files) in enumerate(sorted(controllers.items())):
                    config_files = filtered_config_files[scenario][velocity][controller]
                    rt_files = filtered_rt_files[scenario][velocity][controller]
                    all_x, all_y = [], []
                    all_tar_x, all_tar_y = [], []
                    ...
                    for config_file, mpc_file, rt_file in zip(config_files, mpc_files, rt_files):
                        ...
                        current_sts = [item["current_enu"] for item in data if "current_enu" in item]
                        ...
                        folder_path = os.path.join(self.directory, f"['{scenario}']")
                        self.ensure_directory_exists(folder_path)
                        gif_path = os.path.join(folder_path, f'trajectory_{scenario}_{controller}_{velocity}.gif')
            ...
            for velocity, controllers in sorted(velocities.items()):
                for i, (controller, mpc_files) in enumerate(sorted(controllers.items())):
                    ...
                    for config_file, mpc_file, rt_file in zip(config_files, mpc_files, rt_files):
                        ...
                        current_sts = [item["current_enu"] for item in data if "current_enu" in item]
                        obs_sts = [item["obs_states"] for item in data if "obs_states" in item]
                        current_sts = np.array(current_sts)
            ...
            for velocity, controllers in sorted(velocities.items()):
                for i, (controller, mpc_files) in enumerate(sorted(controllers.items())):
                    ...
                    for config_file, mpc_file, rt_file in zip(config_files, mpc_files, rt_files):
                        ...
                        save_path = os.path.join(folder_path, f'trajectory_{scenario}_{controller}_{velocity}.jpg')
    ...
    def plot_static_traj(self, filtered_mpc_files, filtered_rt_files, filtered_config_files):
        for scenario, velocities in filtered_mpc_files.items():
            for velocity, controllers in sorted(velocities.items()):
                ...
                config_files = filtered_config_files[scenario][velocity][any_controller]  # Pick one controller's config
                ...
                folder_path = os.path.join(self.directory, f"['{scenario}']")
                self.ensure_directory_exists(folder_path)  # Create folder if not exist
                save_path = os.path.join(folder_path, f'trajectory_{scenario}_{velocity}.jpg')
                plt.savefig(save_path, dpi=300, format='jpg', bbox_inches='tight', transparent=False, pad_inches=0.2)
                plt.close(fig)
    ...
    def plot_acc_steer_vel_values(self, filtered_mpc_files, filtered_rt_files, filtered_config_files):
        ...
        for scenario, velocities in filtered_mpc_files.items():
            for velocity, controllers in sorted(velocities.items()):
                ...
                save_path1 = os.path.join(folder_path, f'Compare_acc_{scenario}_{velocity}.jpg')
                save_path2 = os.path.join(folder_path, f'Compare_vel_{scenario}_{velocity}.jpg')
                save_path3 = os.path.join(folder_path, f'Velocity_{scenario}_{velocity}.jpg')
    ...
    def plot_h_values(self, filtered_mpc_files, filtered_rt_files, filtered_config_files):
        ...
        for scenario, velocities in filtered_mpc_files.items():
            for velocity, controllers in sorted(velocities.items()):
                ...
                save_path = os.path.join(folder_path, f'Compare_h_{scenario}_{velocity}.jpg')
    ...
    def plot_h_values_gif(self, filtered_mpc_files, filtered_rt_files, filtered_config_files):
        ...
        for scenario, velocities in filtered_mpc_files.items():
            for velocity, controllers in sorted(velocities.items()):
                ...
                # Save animation as GIF
                gif_path = os.path.join(folder_path, f'Compare_h_{scenario}_{velocity}.gif')
                ani.save(gif_path, writer=PillowWriter(fps=10))
                plt.close(fig)
            ...
            for velocity, controllers in sorted(velocities.items()):
                for controller, mpc_files in sorted(controllers.items()):
                    ...
                    for config_file, mpc_file, rt_file in zip(config_files, mpc_files, rt_files):
                        ...
                        current_sts = [item["current_enu"] for item in rt_data if "current_enu" in item]
                        obs_sts = [item["obs_states"] for item in rt_data if "obs_states" in item]
                        current_sts = np.array(current_sts)  # Take the first entry from the list
                        obs_sts = np.array(obs_sts)  # Take the first entry from the list
            ...
            for velocity, controllers in sorted(velocities.items()):
                for controller, mpc_files in sorted(controllers.items()):
                    ...
                    for config_file, mpc_file, rt_file in zip(config_files, mpc_files, rt_files):
                        ...
                        # Define the correct file path
                        save_path = os.path.join(folder_path, f'optimal_{scenario}_{controller}_{velocity}.jpg')
    ...
    def mpc_error_plot(self, filtered_mpc_files, filtered_rt_files, filtered_config_files):
        for scenario, velocities in filtered_mpc_files.items():
            for velocity, controllers in sorted(velocities.items()):
                ...
                save_path1 = os.path.join(folder_path, f'MPC_errorx_{scenario}_{velocity}.jpg')
                save_path2 = os.path.join(folder_path, f'MPC_errory_{scenario}_{velocity}.jpg')
                save_path3 = os.path.join(folder_path, f'MPC_errorpsi_{scenario}_{velocity}.jpg')
    ...
    def mpc_error_plot_gif(self, filtered_mpc_files, filtered_rt_files, filtered_config_files):
        for scenario, velocities in filtered_mpc_files.items():
            for velocity, controllers in sorted(velocities.items()):
                ...
                gif_path1 = os.path.join(folder_path, f'MPC_x_{scenario}_{velocity}.gif')
                gif_path2 = os.path.join(folder_path, f'MPC_y_{scenario}_{velocity}.gif')
                gif_path3 = os.path.join(folder_path, f'MPC_psi_{scenario}_{velocity}.gif')
                gif_path4 = os.path.join(folder_path, f'MPC_cost_{scenario}_{velocity}.gif')
                ...
                jpg_path1 = os.path.join(folder_path, f'MPC_x_{scenario}_{velocity}.jpg')
                jpg_path2 = os.path.join(folder_path, f'MPC_y_{scenario}_{velocity}.jpg')
                jpg_path3 = os.path.join(folder_path, f'MPC_psi_{scenario}_{velocity}.jpg')
                jpg_path4 = os.path.join(folder_path, f'MPC_cost_{scenario}_{velocity}.jpg')
